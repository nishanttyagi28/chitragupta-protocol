...
```bash
pip install karmasakshi-protocol            # core (CLI, engine, adapters)
pip install karmasakshi-protocol[api]       # + FastAPI control plane + console
pip install karmasakshi-protocol[redis]     # + distributed grant store
pip install karmasakshi-protocol[langgraph] # + LangGraph integration
pip install karmasakshi-protocol[all]       # everything
```
...
`KARMASAKSHI_PUBLIC_DEMO=1` mounts an additional, entirely separate router
at `/demo/*` (see `karmasakshi.web.demo_router`) meant to be safe to expose
...
- Operates on its own isolated, in-memory `DemoSession` (never `ApiState`)
  that auto-resets on a timer (`KARMASAKSHI_DEMO_TTL_SECONDS`) and can be
...
- The authenticated control-plane API (`/manifests`, `/kill-switch`, …)
  ...
  `KARMASAKSHI_PUBLIC_DEMO=1` and `KARMASAKSHI_API_DEV_MODE=1` are both
...
1. Sign in to [render.com](https://render.com) (GitHub login works) and
   connect the `nishanttyagi28/karmasakshi-protocol` GitHub repository.
...
3. Click **Apply** to create the service. No manual environment variable
   ...
   `KARMASAKSHI_PUBLIC_DEMO=1` and leaves `KARMASAKSHI_API_DEV_MODE`/
   `KARMASAKSHI_API_TOKEN` unset.
...
set `KARMASAKSHI_PUBLIC_DEMO=1`, and leave dev-mode/token unset.
...
The dev-mode key generation (`karmasakshi key generate`,
