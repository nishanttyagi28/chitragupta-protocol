...
def doctor(ctx: typer.Context) -> None:
    ...
    def _workspace_check() -> str:
        if not workspace.is_initialized():
            raise KarmaSakshiError(f"workspace not initialized at {workspace.root}")
        return str(workspace.root)

    def _keys_check() -> str:
        key_ids = workspace.list_key_ids()
        if not key_ids:
            raise KarmaSakshiError("no keys found; run `karmasakshi key generate <id>`")
    ...
    def _clock_check() -> str:
        ...
        if now.tzinfo is None:
            raise KarmaSakshiError("system clock is not timezone-aware")
        return f"UTC now: {now.isoformat()}"

    def _adapters_check() -> str:
        from karmasakshi.adapters.email_sandbox import EmailSandboxAdapter
