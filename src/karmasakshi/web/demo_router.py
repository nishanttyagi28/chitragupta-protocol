...
Mounted at ``/demo`` only when ``KARMASAKSHI_PUBLIC_DEMO=1`` is explicitly
set (see :func:`karmasakshi.api.app.create_app`). This router is
intentionally isolated from the real control-plane API and console:

* it operates on its own auto-resetting :class:`~karmasakshi.web.demo_state.DemoSession`,
  never on ``ApiState``;
...
* the real authenticated API/console routes are unaffected and remain
  fail-closed exactly as documented in ``karmasakshi.api.auth``.
"""
...
from karmasakshi.web.demo_scenarios import SCENARIO_ORDER, SCENARIO_TITLES, SCENARIOS
from karmasakshi.web.demo_state import (
    AGENT,
    APPROVER,
    get_session,
    reset_session,
...
from karmasakshi.web.rate_limit import rate_limit_dependency
