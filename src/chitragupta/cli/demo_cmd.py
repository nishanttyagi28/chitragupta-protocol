"""``chitragupta demo --all``: a deterministic, readable walkthrough of every
...
from chitragupta.cli.common import console
from chitragupta.config.clock import FixedClock
from chitragupta.crypto.keyring import Keyring
from chitragupta.crypto.keys import generate_signing_key
...
def _build_context() -> DemoContext:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="chitragupta-demo-"))
    return DemoContext(
        now=now,
        clock=clock,
        engine=engine,
...
def scenario_6_and_7_delegation(ctx: DemoContext) -> None:
    ...
    from chitragupta.errors import ConstraintWideningError
    from chitragupta.grants.issuer import issue_grant
...
def scenario_10_tampered_signature(ctx: DemoContext) -> None:
    from chitragupta.errors import InvalidSignatureError
...
def scenario_12_outcome_mismatch(ctx: DemoContext) -> tuple[Any, Any, Any, Any]:
    ...
    from chitragupta.domain.common import AdapterIdentity
...
def run_demo() -> bool:
    ctx = _build_context()
    console.print("[bold]Chitragupta Protocol -- deterministic demonstration suite[/bold]\n")
