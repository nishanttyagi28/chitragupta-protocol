...
Deliberately separate from :class:`~chitragupta.api.state.ApiState` (the real
...
DEMO_TTL_ENV = "CHITRAGUPTA_DEMO_TTL_SECONDS"
...
def build_session() -> DemoSession:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="chitragupta-public-demo-"))
    adapters: dict[str, EffectAdapter] = {
...
def get_session() -> DemoSession:
    ...
    aged past ``CHITRAGUPTA_DEMO_TTL_SECONDS`` (default 15 minutes)."""
    global _current
    with _lock:
