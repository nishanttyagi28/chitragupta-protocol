...
def verify_seal(sealed: SealedManifest, keyring: Keyring) -> None:
    ...
    Raises :class:`~chitragupta.errors.ManifestTamperedError` if the manifest
    was mutated after sealing, :class:`~chitragupta.errors.UnknownKeyError`
    if the seal references a key not in ``keyring``, or
    :class:`~chitragupta.errors.InvalidSignatureError` if the signature does
